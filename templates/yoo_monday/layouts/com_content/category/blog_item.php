...
$args    = include(__DIR__.'/../../../warp/systems/joomla/layouts/com_content/article_defaults.php');
...
if ($params->get('show_tags', 1) && !empty($this->item->tags->itemTags)) {
	...
	foreach ($this->item->tags->itemTags as $i => $tag) {
		if (in_array($tag->access, JAccess::getAuthorisedViewLevels(JFactory::getUser()->get('id')))) {
			...
			$tags[$i]['title'] = $this->escape($tag->title);
			$tags[$i]['link'] = '<a href="'.JRoute::_(TagsHelperRoute::getTagRoute($tag->tag_id . ':' . $tag->alias)).'">'.$this->escape($tag->title).'</a>';
