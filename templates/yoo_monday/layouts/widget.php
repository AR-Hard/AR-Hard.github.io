...
foreach (array('suffix', 'panel', 'title_size', 'center', 'class', 'badge', 'icon', 'display', 'panel_header', 'panel_padding') as $var) {
    $$var = isset($params[$var]) ? $params[$var] : null;
}

// Set default panel
if ($panel == '' && in_array($widget->position, array( 'top-a', 'top-b', 'top-c', 'top-d', 'bottom-a', 'bottom-b', 'bottom-c','bottom-d', 'main-top', 'block-footer', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer-right', 'footer-left', 'offcanvas'))) {
    $panel = 'uk-panel';
}
...
// Set title
if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer-right', 'footer-left'))) {
    $title = '';
...
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
    ...
    if ($panel_padding) $classes[] = "tm-panel-padding-large";
