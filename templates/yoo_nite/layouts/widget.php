...
// Set default panel
if ($panel == '' && in_array($widget->position, array('top-a', 'top-b', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}

// Set panel for specific positions
else if (in_array($widget->position, array('headerbar', 'fullscreen', 'footer', 'offcanvas'))) {
    $panel = 'uk-panel';
}

// Set panel for toolbar position
else if (in_array($widget->position, array('toolbar'))) {
    $panel = 'uk-panel uk-panel-blank';
}
...
// Set title
if (in_array($widget->position, array('headerbar', 'toolbar', 'footer'))) {
    $title = '';
...
if ($widget->menu) {
    ...
    } else if (in_array($widget->position, array('toolbar', 'footer'))) {
        $renderer = 'subnav';
        $widget->nav_settings["modifier"] = "uk-subnav-line uk-flex-center";
