...
foreach ($blocks as $block) {
    ...
    $block_classes[$block] = $config->get("block.{$block}.block-divider") ? 'tm-position-divider ' : '';
...
$body_classes .= $config->get('navbar_animated') ? ' tm-navbar-animated' : '';
...
if ($this['useragent']->browser() == 'msie') {
    $head[] = sprintf('<!--[if lte IE 10]><link rel="stylesheet" href="%s"><![endif]-->', $this['path']->url('css:ie.css'));
