...
// Set default panel
if ($panel == '' && in_array($widget->position, array('header', 'top-a', 'top-b', 'top-c', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'bottom-e', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
if ($panel == '' && in_array($widget->position, array('footer-top', 'footer-bottom', 'footer', 'offcanvas'))) {
    $panel = 'uk-panel';
}
...
// Set title
if (in_array($widget->position, array('footer'))) {
    $title = '';
...
if ($widget->menu) {
    ...
    } else if (in_array($widget->position, array('footer'))) {
        $renderer = 'subnav';
        $widget->nav_settings["modifier"] = "uk-subnav-line";
    } else if (in_array($widget->position, array('footer-top', 'footer-bottom'))) {
        $renderer = 'subnav';
        $widget->nav_settings["modifier"] = "tm-subnav-bullet";
...
// Render widget
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'smoothscroll', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
    echo $content;
    ...
    if ($widget->menu) {
        ...
        <ul class="uk-navbar-nav uk-hidden-small">
            <li class="uk-parent" data-uk-dropdown>
                <a href="#">'.$icon.$title.'</a>
                <div class="uk-dropdown uk-dropdown-center uk-dropdown-navbar">'.$content.'</div>
            </li>
        </ul>';
    }
} else {
