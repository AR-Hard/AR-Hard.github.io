...
$positions = array_merge(array_keys($config->get('grid', array())), array('fullscreen'));
...
foreach ($blocks as $block) {
    ...
    if ($this['config']->get("block.{$block}.block_full", false)) {
        $block_classes[$block][] = 'tm-block-full';
    }
...
$body_classes .= $config->get('navbar_fixed') ? ' tm-navbar-fixed' : '';
$body_classes .= $config->get('navbar_transparent') ? ' tm-navbar-transparent' : '';
$body_classes .= $config->get('block_space') ? ' tm-block-large' : '';
